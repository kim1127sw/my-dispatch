...
st.markdown("""
    <style>
        .block-container { padding-top: 0.8rem; padding-bottom: 1rem; padding-left: 0.6rem; padding-right: 0.6rem; max-width: 100%; }
        ...
        /* 입력창 스타일 모바일 최적화 */
        input[type="text"] { font-size: 16px !important; padding: 8px !important; }
...
# 1. 제목 크기 모바일용으로 축소
st.markdown("<h3 style='font-size: 19px; font-weight: bold; color: #1e293b; margin-bottom: 12px;'>🚚 감사 페스티벌 배차 조회</h3>", unsafe_allow_html=True)
...
if error_msg:
    ...
    # 2. 차량번호 입력 (즉시 조회)
    search_query = st.text_input("🔍 차량번호 4자리 입력", "", placeholder="숫자 4자리 입력 후 화면 터치")

    if not search_query.strip():
        st.info("👆 위 입력창에 본인의 차량번호 4자리를 입력해주세요.")
    else:
        ...
        if filtered_df.empty:
            ...
            st.markdown("👇 **목록을 클릭하시면 하단에 배송 품목이 표시됩니다.**")
            
            # 3. 표 행 클릭(터치) 시 즉시 선택 이벤트 감지
            event = st.dataframe(
                display_df, 
            ...
            # 표 클릭이 없더라도 쉽게 선택할 수 있는 1클릭 라디오 버튼
            ...
            if selected_delivery:
                ...
                <div class="custom-card">
                    ...
                    <b>📅 생성일자:</b> {row_data['SOCreationDate']}
                </div>
                """, unsafe_allow_html=True)
