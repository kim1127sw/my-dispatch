...
# 모바일 최적화 여백 및 글자 크기 (상단 여백 padding-top을 3.5rem으로 대폭 늘려 글자 잘림 방지)
st.markdown("""
    <style>
...
if error_msg:
    ...
    if not search_query.strip():
        ...
        if filtered_df.empty:
            ...
            display_df = display_df.rename(columns={'SOCreationDate': '생성일자', 'ShipToAddress': '주소'})
            ...
            if selected_delivery:
                ...
                with st.container(border=True):
                    ...
                    st.markdown(f"**📅 주문생성일자:** {row_data['SOCreationDate']}")
