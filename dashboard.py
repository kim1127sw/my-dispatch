...
st.markdown("""
    <style>
        ...
        /* 모바일 입력창 폰트 크기 확대 (포커스 시 확대 현상 방지) */
...
def load_and_process_data():
    ...
    car_to_cell = {}
    for col_idx in range(len(df_mapping.columns)):
        current_cell = "미분류"
        for row_idx in range(len(df_mapping)):
            val = str(df_mapping.iloc[row_idx, col_idx]).strip()
            if not val or val.lower() in ('nan', 'none', 'nat'): continue
            clean_val = val.replace(" ", "").replace(".0", "")
            
            if not clean_val.isdigit() and len(clean_val) <= 10 and not any(k in clean_val for k in ['경북', '서울', '경기', '부산', '대구', '인천', '광주', '대전', '울산', '충청', '전라', '경상', '강원', '제주']):
                if len(clean_val) >= 2 and not ('-' in clean_val or '010' in clean_val):
                    current_cell = val
            
            if len(clean_val) >= 4 and any(char.isdigit() for char in clean_val):
                four_digit = clean_val[-4:] if clean_val[-4:].isdigit() else ""
                if current_cell != "미분류":
                    car_to_cell[clean_val] = current_cell
                    if four_digit: car_to_cell[four_digit] = current_cell
    ...
    df_filtered['SOCreationDate'] = df_filtered['SOCreationDate'].dt.strftime('%m-%d') # 월-일만 표시하여 폭 줄이기
...
if error_msg:
    ...
    if not search_query:
        ...
        if filtered_df.empty:
            ...
            # 모바일 화면 폭에 맞게 핵심 열만 보기 좋게 압축 (주소는 너무 길면 가독성이 떨어지므로 요약)
