...
st.markdown("""
    <style>
        .block-container { padding-top: 3.5rem; padding-bottom: 1rem; padding-left: 0.8rem; padding-right: 0.8rem; max-width: 100%; }
...
# 1. 폰에서 잘리지 않는 적절한 크기의 제목
st.markdown("### 🚚 감사 페스티벌 배차 조회")
...
if error_msg:
    ...
    # ✨ 추가된 부분: 전체 대상 건수를 눈에 띄게 표시
    st.info(f"📊 **감사페스티벌 전체 배차 대상: 총 {len(df):,}건**")
    ...
    if not search_query.strip():
        st.markdown("<div style='font-size: 13px; color: #64748b; margin-top: 5px;'>👆 빈칸에 본인의 차량번호 4자리를 입력하고 돋보기 버튼을 눌러주세요.</div>", unsafe_allow_html=True)
    else:
        ...
        if filtered_df.empty:
            ...
            st.success(f"📍 소속: **{my_cell}** (총 {len(filtered_df)}건 검색됨)")
            ...
            if selected_delivery:
                ...
                # 다크모드/라이트모드 모두 글씨가 완벽하게 보이는 전용 컨테이너
