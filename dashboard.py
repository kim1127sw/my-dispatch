...
def load_and_process_data():
    ...
    # =================================================================
    # [무적 매핑 로직] 엑셀 열 번호를 무시하고, 실제 값의 위치로 짝짓기
    # =================================================================
    car_to_cell = {}
    for _, row in df_mapping.iterrows():
        # 해당 행에서 빈 칸(NaN)을 모두 제거하고 글자가 있는 칸만 리스트로 모음
        row_vals = [str(v).strip() for v in row.values if pd.notna(v) and str(v).strip().lower() not in ('nan', 'none', '')]
        
        # 값이 2개 이상 있는 행만 취급 (최소 차번 1개 + 소속셀 1개)
        if len(row_vals) >= 2:
            # 1. 무조건 첫 번째 값이 전체 차번 (예: 경북86아6791)
            full_car = row_vals[0].replace(" ", "").replace(".0", "")
            # 2. 무조건 가장 마지막 값이 소속셀 (예: 하누리)
            cell_name = row_vals[-1]
            
            if len(full_car) >= 4 and not cell_name.isdigit():
                car_to_cell[full_car] = cell_name
                # 뒤에서 4자리도 같이 등록
                if full_car[-4:].isdigit():
                    car_to_cell[full_car[-4:]] = cell_name
                    
                # 중간에 섞여 있는 숫자 4자리(예: '6791')도 혹시 모르니 찾아서 등록
                for v in row_vals:
                    v_clean = v.replace(".0", "")
                    if v_clean.isdigit() and len(v_clean) >= 4:
                        car_to_cell[v_clean[-4:]] = cell_name
